...
export default function Katalog2DetailPage() {
    ...
    useEffect(() => {
        const load = async () => {
            setLoading(true);
            try {
                const res = await fetch(`/api/katalog2/${id}`, { cache: 'no-store' });
                if (!res.ok) throw new Error('Failed to fetch');
                const data = await res.json();
                setProduct(data);
                if (data.colorMappings) setColorMappings(data.colorMappings);

                // Check URL color param
                ...
                if (data.variants && data.variants.length > 0) {
                    ...
                    if (urlColor) {
                        ...
                        if (matchingVariant) initialVariant = matchingVariant;
                    }
    ...
    if (!product) return (
        <div className="min-h-screen bg-zinc-50 flex items-center justify-center">
            <div className="text-center">
                ...
                <Link href="/katalog2" className="text-primary hover:underline">Zpět do katalogu</Link>
            </div>
        </div>
    );
    ...
    const prices = variantsForPrice.map(v => v.price).filter(p => p > 0);
    ...
    if (!priceDisplay) {
        if (minPrice === 0 && maxPrice === 0) {
            priceDisplay = <span className="text-zinc-400">Cena na vyžádání</span>;
        } else if (minPrice === maxPrice) {
            priceDisplay = new Intl.NumberFormat('cs-CZ', { style: 'currency', currency: 'CZK', maximumFractionDigits: 0 }).format(minPrice);
        } else {
            priceDisplay = (
                <>
            ...
            <div className="bg-white border-b border-zinc-200">
                <div className="container-custom py-3">
                    <nav className="flex text-sm text-zinc-500">
                        ...
                        <Link href="/katalog2" className="hover:text-zinc-900 transition-colors">Katalog V2</Link>
                        <span className="mx-2">/</span>
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            ...
                            {/* Status badge based on selected color */}
                            {(() => {
                                const colorVars = product.variants.filter(v => v.color === selectedColor);
                                const hasStockInColor = colorVars.some(v =>
                                    v.b2bOrderStatus === 'skladem' ||
                                    (Number(v.stock) || Number(v.onHand) || Number(v.qty) || Number(v.b2bStockQuantity) || 0) > 0
                                );
                                const hasTransitInColor = !hasStockInColor && colorVars.some(v => v.b2bOrderStatus === 'na_ceste');
                                const hasOrderInColor = !hasStockInColor && !hasTransitInColor && colorVars.some(v => v.b2bOrderStatus === 'na_objednavku');

                                if (hasStockInColor) return (
                                    <div className="absolute top-4 left-4">
                                        <span className="text-xs font-bold px-3 py-1.5 rounded-lg bg-green-100 text-green-700 border border-green-200 uppercase shadow-sm">Skladem</span>
                                    </div>
                                );
                                if (hasTransitInColor) return (
                                    <div className="absolute top-4 left-4">
                                        <span className="text-xs font-bold px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700 border border-blue-200 uppercase shadow-sm">Na cestě</span>
                                    </div>
                                );
                                if (hasOrderInColor) return (
                                    <div className="absolute top-4 left-4">
                                        <span className="text-xs font-bold px-3 py-1.5 rounded-lg bg-zinc-100 text-zinc-500 border border-zinc-200 uppercase shadow-sm">Na objednávku</span>
                                    </div>
                                );
                                return null;
                            })()}
                                            ...
                                            >
                                                {image ? (
                                                    <>
                                                        ...
                                                        />
                                                        {(() => {
                                                            const hex = colorMappings[color] || guessHexFromName(color);
                                                            if (!hex) return null;
                                                            return (
                                                                <span
                                                    ...
                                                    <div className="w-full h-full flex flex-col items-center justify-center bg-zinc-50 p-1">
                                                        {(() => {
                                                            const hex = colorMappings[color] || guessHexFromName(color);
                                                            return hex ? (
                                                                <>
                                                                    <span
                                                                        className="w-8 h-8 rounded-full border border-zinc-200 mb-1"
                                                                        style={{ backgroundColor: hex }}
                                                                    />
                                                                    <span className="text-[9px] text-zinc-400 text-center leading-tight line-clamp-2">{color}</span>
                                                                </>
                                                            ) : (
                                                                <span className="text-xs text-zinc-400 text-center">{color}</span>
                                                            );
                                                        })()}
                                                    </div>
                                                )}
                        ...
                        <div className="bg-white rounded-2xl border border-zinc-100 p-6 shadow-sm mb-8">
                            ...
                            <div className="flex flex-wrap gap-2 mb-4">
                                {availableSizes.map((size) => {
                                    const sizeVariants = variantsInFrame.filter(v =>
                                        standardizeSize(v.size, category) === size &&
                                        (!selectedCapacity || v.capacity === selectedCapacity)
                                    );

                                    const inStockVariant = sizeVariants.find(v => {
                                        const s = Number(v.stock) || Number(v.onHand) || Number(v.qty) || Number(v.b2bStockQuantity) || 0;
                                        return s > 0 || v.b2bOrderStatus === 'skladem';
                                    });

                                    const inTransitVariant = sizeVariants.find(v => v.b2bOrderStatus === 'na_ceste');
                                    const onOrderVariant = sizeVariants.find(v => v.b2bOrderStatus === 'na_objednavku');

                                    const variant = inStockVariant || inTransitVariant || onOrderVariant || sizeVariants[0];
                                    ...
                                    const inTransit = !inStock && !!inTransitVariant;
                                    const onOrder = !inStock && !inTransit && !!onOrderVariant;
                                    const isSelected = selectedSize === size;

                                    if (!variant && selectedCapacity) return null;

                                    return (
                                        <div key={size} className="relative group">
                                            <button
                                                onClick={() => setSelectedSize(size)}
                                                className={`h-12 px-4 rounded-lg border-2 text-sm font-medium transition-all flex items-center gap-2
                                                    ${inStock
                                                        ? `border-green-200 bg-green-50 text-green-800 ${isSelected ? 'ring-2 ring-green-600 ring-offset-1' : 'hover:border-green-300'}`
                                                        : inTransit
                                                            ? `border-blue-200 bg-blue-50 text-blue-800 ${isSelected ? 'ring-2 ring-blue-600 ring-offset-1' : 'hover:border-blue-300'}`
                                                            : onOrder
                                                                ? `border-zinc-300 bg-zinc-50 text-zinc-600 ${isSelected ? 'ring-2 ring-zinc-500 ring-offset-1' : 'hover:border-zinc-400'}`
                                                                : `border-zinc-200 bg-white text-zinc-900 ${isSelected ? 'ring-2 ring-zinc-900 ring-offset-1' : 'hover:border-zinc-300 hover:shadow-sm'}`
                                                    }
                                                `}
                                            >
